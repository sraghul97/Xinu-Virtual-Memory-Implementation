...
extern pid32 newpid();
extern void write_pdbr(unsigned long);
extern syscall vfree(char*, uint32);
extern void init_pte(pt_t *, unsigned int, unsigned int, unsigned int);
extern void init_pde(pd_t *, unsigned int, unsigned int);
extern void split_vaddr(virt_addr_t*, unsigned int);
extern uint32 find_free_PDPT();
extern uint32 find_free_FFS();
extern uint32 find_free_swap();
extern uint32 free_ffs_pages();
extern uint32 free_swap_pages();
extern void dealloc_pdptffs(pd_t*);
...
extern uint32 find_FFS_to_evict();
extern bool8 perform_eviction(pd_t *, uint32, uint32);
extern void free_swap_space(pid32);
