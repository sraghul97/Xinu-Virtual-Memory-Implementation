...
static	void	sysinit()
{
	...
	// Link the page fault handler interrupt
	...
	// Create PTEs to map Xinu's space
	pte = (pt_t*) (maxheap + 1);
	for (i = 0; i < XINU_PAGES+1024; i++)
	{
		init_pte(&pte[i], 1, 1, (i*PAGE_SIZE) >> 12);
	}

	// Create page directories for Xinu's space
	pde = (pd_t*) SYSTEM_PD_BASE;
	for (i = 0; i < 9; i++)
	{
	...
	// for (i = 0; i < 8; i++)
	// {
	// 	kprintf("PDE[%d] is at %x and pointing to PTE at %x\n", i, &pde[i], pde[i].pd_base);
	// 	for (j = 0; j < 4; j++)
	// 	{
	// 		kprintf("\tPTE[%d]->%x\n", j, pte[i*1024 + j].pt_base);
	// 	}
	// }

	// Write CR3 with the base address of the PD
	write_pdbr(PDPT_BASE + PAGE_SIZE*8);
